...
- 자재관리와 레이어관리의 브라우저 저장값은 `localStorage`에 저장됩니다. 다른 PC나 아이폰에서 임시로 같은 설정을 쓰려면 앱의 설정 탭에서 `다른 기기로 복사 / 복사한 설정 불러오기`를 사용합니다.
- 사용자가 설정 탭의 `공장값 설정하기`로 복사한 내용을 보내면, 아래 명령으로 현재 설정을 코드 기본값에 자동 반영할 수 있습니다. 이후 배포하면 아이폰과 다른 PC에서도 같은 기본값이 바로 보입니다.

```bash
pnpm apply-factory
```

- 파일로 받은 경우에는 아래처럼 실행합니다.

```bash
node scripts/apply-factory-settings.mjs path/to/factory-settings.json
```

- 공장 초기화는 비밀번호가 필요합니다. 기본 비밀번호는 `reset`이며 설정 탭에서 변경할 수 있습니다.
